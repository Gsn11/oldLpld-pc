...
export class ExamineComponent implements OnInit {
	...
	openUserModelDialog() {
		...
		dialogRef.afterClosed().subscribe(result => {
			if (result) {
				this.overhaulPersonList = result[0].selected;
			} else { }
		});
	}
	...
	confirm() {
		...
		if (this.overhaulPersonList.length === 0) {
			this.snackBar.open('请选择检修或技术组班长', '确认', {
				duration: 1600,
			...
			this.overhaulPersonList.forEach(item => {
				data.RepairMonitors += (item.Seq + ',');
			});
			data.RepairMonitors = data.RepairMonitors.substring(0, data.RepairMonitors.length - 1);
		}

		this.service.serviceR('ent/maintenance/dnsc/8004', data, (res: any) => {
			if (res.ResultCode === 0) {
