...
@Component({
	selector: 'app-sche-dialog',
	templateUrl: './sche-dialog.component.html',
	styleUrls: ['./sche-dialog.component.scss'],
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class ScheDialogComponent implements OnInit {
	...
	constructor(
		...
		public dialogRef: MatDialogRef<ScheDialogComponent>,
		@Inject(MAT_DIALOG_DATA) public data: DialogData,
		private cdr: ChangeDetectorRef
	) {
		this.selectSeq = null;
		this.uData = data.Schedule;
		this.type = data.type;
		this.PageIndex = 1;
		this.PageSize = 10;
		this.displayedColumns = ['MSName', 'TimeType', 'MsDesc'];
	...
	ngOnInit() {
		this.Schedules = this.uData;
		console.log(this.Schedules);
		for (const i of this.Schedules) {
			if (i.TimeType === 0) {
				Reflect.set(i, 'TimeType', '单次');
			} else if (i.TimeType === 1) {
				Reflect.set(i, 'TimeType', '每日');
			} else if (i.TimeType === 2) {
				Reflect.set(i, 'TimeType', '每周');
			} else if (i.TimeType === 3) {
				Reflect.set(i, 'TimeType', '每月');
			} else if (i.TimeType === 4) {
				Reflect.set(i, 'TimeType', '每季度');
			} else if (i.TimeType === 5) {
				Reflect.set(i, 'TimeType', '每年');
			}
		}
	}
