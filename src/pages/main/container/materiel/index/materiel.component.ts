...
export class MaterielComponent implements OnInit {
	...
	// 物料导出
	materielExpot() {
		new DownloadFile({ CommonSearch: this.searchVal, Stat: '0,1' }, 'ent/material/6205').downloadfile();
	}
	...
	goPage(type, el) {
		switch (type) {
			case 'add':
				this.router.navigate(['index/materiel/add']);
				break;
			case 'edit':
				sessionStorage.setItem('materielItem', JSON.stringify(el));
				this.router.navigate(['index/materiel/edit']);
				break;
			case 'record':
				sessionStorage.setItem('materielItem', JSON.stringify(el));
				this.router.navigate(['index/materiel/record']);
				break;
			case 'look':
				sessionStorage.setItem('materielItem', JSON.stringify(el));
				this.router.navigate(['index/materiel/look']);
				break;
		}
	}
}
