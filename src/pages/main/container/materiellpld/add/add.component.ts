...
export class AddMaterielComponent implements OnInit {
	...
	openBuildDialog(): void {
		const data = {type:'',buildSeq:''}
		if(this.type === 'material'){
			data.type = null;
			data.buildSeq =null;
		}else{
			data.type = null;
			data.buildSeq = this.type ? buildData.materielVal[this.type].Building : null;
		}
	...
	addMateriel() {
		...
		const data: any = {
			...
			//
			Thresholds: this.threshold, // 阈值
			Mobile: '',
			// Avatar: this.imgsrcData.length > 0 ? this.imgsrcData[0].ImgUrl : ''
		};
		...
		if (!data.Building) {
			this.snackBar.open('请点击选择建筑物', '确认', {
				duration: 1600,
				verticalPosition: 'top',
				panelClass: 'snack-bar-color-danger'
			});
		...
		if (this.imgsrcData.length > 0) {
			data.Avatar = this.imgsrcData[0].ImgUrl;
		}

		this.service.serviceR('ent/material/6202', data, (res: any) => {
			if (res.ResultCode === 0) {
