...
export class devServabilityReportComponent implements OnInit {
	// pageSizeOptions = [5, 10, 20];
	// paginatorTotal = 0;
	// pageIndex = 1;
	// pageSize = 10;
	date: any;
	displayedColumns = ['index', 'subSystem', 'intactTimeNum', 'allTimeNum', 'intactProportion', 'allNum', 'file', 'fileProportion'];
	...
	getList() {
		this.service.serviceR('ent/report/31001', {Date: this.date}, (res: any) => {
			if (res.ResultCode === 0) {
	...
	downloadDeviceFile() {
		...
		new DownloadFile(body, 'ent/report/31001/export').downloadfile();
	}
