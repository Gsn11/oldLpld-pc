...
export class scheduleComponent implements OnInit {
	...
	@ViewChild(DoubleComponent, null) double: DoubleComponent;
	constructor(
	...
	ngOnInit() {
		...
		this.nowToday = d.getDate();
		this.gettime(now);
		const list = this.getDaysInMonth(this.nowYear, this.nowMonth);
		const listr = this.getDaysInMonth(this.nowYear, this.nowMonth);
		this.geiTeam();
		this.data = list;
		this.datao = listr;
		this.getList(now);
	...
	getList(time) {
		...
		this.service.serviceR('workteamSchedule/15001', da, (res: any) => {
			if (res.ResultCode === 0) {
	...
	doubleSelect(){
		this.double.switchModalBox();
	}

	addSelectWeek(id: any, j: any){
		const Time= this.nowYear + '-' + this.nowMonth;
		const newMonth = this.nowMonth - 1 < 10 ? '0' + (this.nowMonth - 1) : this.nowMonth - 1;
		const newTime = this.nowYear + '-' + newMonth;
		this.modal.type = 'add';
		this.modal.dateStr = id.dateStr;
		this.modal.User = this.ocrelist[j].User;
		this.modal.userName ='';
		this.modal.timeStart = '';
		this.modal.timeEnd = '';
		this.modal.week ='';
		this.modal.workTimeAbbrName = '';
		this.modal.keySeq = '';
		this.modal.Seq = -1;
		this.modal.nameSeq =-1;
		if(Time === this.selectTime || new Date(Time) < new Date(this.selectTime)){
			this.modal.switchModalBox();
		}else if(newTime === this.selectTime){
			if( this.nowToday < 4){
				this.modal.switchModalBox();
			}
		}
	...
	selectWeek(id: any, j: any, k: any) {
		...
		this.modal.userName = id.userName;
		this.modal.timeStart = id.timeStart;
		this.modal.timeEnd = id.timeEnd;
		this.modal.dateStr = id.dateStr;
		this.modal.week = id.week;
		this.modal.workTimeAbbrName = id.workTimeAbbrName;
		this.modal.keySeq = id.key;
		this.modal.Seq = id.seq;
		this.modal.nameSeq =Number(id.workTimeSeq);		
		if(Time === this.selectTime || new Date(Time) < new Date(this.selectTime)){
			this.modal.switchModalBox();
		}else if(newTime === this.selectTime){
			if( this.nowToday < 4){
	...
	downloadDeviceFile() {
		...
		new DownloadFile(body, 'workteamSchedule/export').downloadfile();
	}
}
